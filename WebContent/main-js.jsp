<%@ page language="java" contentType="text/javascript; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@ include file="WEB-INF/js/math/math.js" %>
<%@ include file="WEB-INF/js/extensions.js" %>
<%@ include file="WEB-INF/js/math/Statistics.js" %>
<%@ include file="WEB-INF/js/math/EqSolvers.js" %>

<%@ include file="WEB-INF/js/Constants.js" %>
<%@ include file="WEB-INF/js/MainApp.js" %>
<%@ include file="WEB-INF/js/Portfolio.js" %>
<%@ include file="WEB-INF/js/Stock.js" %>
<%@ include file="WEB-INF/js/logic.js" %>

<%@ include file="WEB-INF/js/charts/Charts.js" %>
<%@ include file="WEB-INF/js/charts/LinearChart.js" %>
<%@ include file="WEB-INF/js/charts/PSDChart.js" %>
<%@ include file="WEB-INF/js/charts/QuotesChart.js" %>
